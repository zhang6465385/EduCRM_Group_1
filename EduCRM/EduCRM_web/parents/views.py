...
def Parents_Register(request):
    ...
    if request.method == "POST":
        ...
        if not all([parents_name,username,password,id_card]):
            if not parents_name:
                error_data['parents_name'] = {'error':'账号未填写'}
            if not username:
                error_data["username"] = {'error': '用户名未填写'}
            if not password:
        ...
        try:
            if re.match("[\u4e00-\u9fa5]+",parents_name):
                if len(parents_name) <= 5:
                    u = Parents.objects.filter(username=username).first()
                    if not u:
                        if re.match("^[a-zA-Z][a-zA-Z0-9_]*$",username):
                            if len(username)>=6 and len(username)<=12:
                                            ...
                                            if re.match('^(?=.*\d)(?=.*[A-Z]).{8,10}$', password):
                                                if len(password) >= 8 and len(password)<=12:
                                                    ...
                                                    if password == confirm_password:
                                                        ...
                                                        parents = Parents(parents_name=parents_name,username=username,id_card=id_card,password=passwords)
                                                        parents.save()
                                                        return redirect("/parents_login/")
                                                    else:
                                                        error_data["confirm_password"] = {"error":"两次密码不一样"}
                                ...
                                error_data["username"] = {"error":"用户名长度有误"}
                        else:
                            error_data["username"] = {"error":"用户名证格式有误"}
                    else:
                        error_data["username"] = {"error": "此用账号已经被注册"}
                else:
...
def Parents_Login(request):
    ...
    if request.method == "POST":
        ...
        if not all([username, password]):
            if not username:
                error_data["username"] = {"error":"账号为空"}
            if not password:
            ...
            if u:
                ...
                error_data["username"] = {"error": "此账号未存在"}
